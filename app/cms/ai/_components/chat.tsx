          ...
          <div key={toolCallId}>
            ...
            {toolInvocation?.result && (
              <div className="pt-[0.5rem] flex gap-1">
                <Link href={`https://${toolInvocation?.result?.site_subdomain}.Zain.xyz`} target="_blank">
          ...
          <div key={toolCallId} className='bg-blue-700 bg-opacity-10 text-sm whitespace-pre-wrap px-3 py-2 rounded-lg w-fit'>
            {message && JSON.parse(result).map((info: any) => (
              <div key={info?.site_id}>
                {message + " "}
                {info?.site_name}: <Link href={`https://${info?.site_subdomain}.Zain.xyz`} className="underline" target="_blank">{info?.site_subdomain + ".Zain.xyz"}</Link>
              </div>
            ))}
          </div>
        );
